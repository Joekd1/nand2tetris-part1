...
We first use =and_then= to extract a reference to the currrent instruction:
...
The logic is similar and only the name changes. We filter filter the insruction type and based on the result, we exract the capture of the current instruction using the supplied name.
