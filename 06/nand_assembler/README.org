...
Returns the current valid instruction by using =current_line=. The initial instruction returns None and is updated when a valid instruction if encountered. It doesn't change in case of a comment or empty line.
